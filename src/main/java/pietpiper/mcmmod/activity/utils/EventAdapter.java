...
public class EventAdapter {
  ...
  /**
   * Adapts a command registration event consumer to the raw event parameters.
   *
   * @param consumer the {@link Consumer} that processes the {@link CommandRegistrationRequest}
   * @return a {@link CommandRegistrationCallback} event handler
   */
  public static CommandRegistrationCallback adaptCommandRegistration(Consumer<CommandRegistrationRequest> consumer) {
    return (dispatcher,
            registryAccess,
            environment) ->
            consumer.accept(CommandRegistrationRequest.of(dispatcher));
  }
}